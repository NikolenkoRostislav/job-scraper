...
from api.rate_limiter import rate_limit_token_by_ip, rate_limit_token_by_username
...
@router.post(
    ...
    dependencies=[
        Depends(rate_limit_token_by_username),
        Depends(rate_limit_token_by_ip),
    ],
    responses=responses_for(403, 429),
)
...
@router.post("/refresh", response_model=Token, responses=responses_for(403))
...
async def google_login(request: Request):
    return await oauth.google.authorize_redirect(
        request, settings.auth.GOOGLE_CALLBACK_URL
    )


@router.get("/google/callback", responses=responses_for(403, 409))
...
@router.post("/send/email-code", responses=responses_for(409))
async def send_email_code(
    db: DatabaseDep, background_tasks: BackgroundTasks, receiver: EmailStr
):

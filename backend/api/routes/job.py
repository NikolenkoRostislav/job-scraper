...
async def get_jobs(
    ...
    return await JobService.get_jobs(
        db, page=page, page_size=page_size, order_by=order_by, filters=filters
    )
...
@router.get(
    "/{job_id}/skills", response_model=list[SkillBase], responses=responses_for(404)
)
async def get_job_skills(db: DatabaseDep, job_id: int):
    return await JobService.get_job_skills(db, job_id)


@router.get("/{job_id}", response_model=JobDetailed, responses=responses_for(404))

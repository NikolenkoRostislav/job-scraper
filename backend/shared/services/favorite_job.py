...
class FavoriteJobService:
    ...
    async def favorite_job(db: AsyncSession, user_id: int, job_id: int):
        job = await JobService.get_job_by_id(
            db, job_id
        )  # raises exception if job doesnt exist

        try:
            favorited_job = FavoritedJobListing(user_id=user_id, job_listing_id=job_id)
    ...
    async def unfavorite_job(db: AsyncSession, user_id: int, job_id: int):
        result = await db.scalars(
            select(FavoritedJobListing).where(
    ...
    async def get_favorited_jobs(db: AsyncSession, user_id: int, filters: JobFilters):
        stmt = (
            ...
            .where(FavoritedJobListing.user_id == user_id)
        )

...
from shared.schemas import DateRange, WebsiteStats, LogEntry
from shared.utils import LogLevel, LOG_LEVEL_PRIORITY, get_log_file
...
class StatsService:
    @staticmethod
    async def get_logs(
        log_name: str, log_level: LogLevel, date_range: DateRange
    ) -> list[LogEntry]:
        """
        ...
        for log_str in log_str_list:
            ...
            timestamp = datetime.strptime(
                log_date_str.strip(), "%Y-%m-%d %H:%M:%S,%f"
    ...
    @staticmethod
    async def get_stats(
        db: AsyncSession, source_website: str, date_range: DateRange
    ) -> WebsiteStats:
        """
        ...
        scrape_reports = await ScrapeReportService.get_scrape_reports(
            db, source_website, date_range=date_range, failed_only=False
        )
        scrape_count = len(scrape_reports)
        failed_scrape_count = len(
            [
                scrape_report
                for scrape_report in scrape_reports
                if (scrape_report.end_reason != "finished")
            ]
        )
